...
export default class GamepadApi extends HTMLElement {
  ...
  _checkButtons() {
    const prev = this.pad(0).buttons[6].pressed ? 'prev' : null;
    const next = this.pad(0).buttons[7].pressed ? 'next' : null;
    if (prev || next) {
  ...
  detectPad() {
    window.addEventListener("gamepadconnected", (e) => {
      ...
      console.log(e.gamepad, navigator.getGamepads());
      this.setPad(e.gamepad);
    });
  }
